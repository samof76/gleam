...
pub fn items_middleware(
  ...
  let parsed_items = {
    case wisp.get_cookie(req, "items", wisp.PlainText) {
      Ok(json_string) -> {
        // define a `decoder` that would decode a Json
        // and then parse into a list of Json strings
        let decoder =
          dynamic.decode3(
            ItemsJson,
        ...
        // Pass the `decoder` to parse the `json_string`
        // obtained from the cookie
        let result = json.decode(json_string, decoder)
        // On `Error` pass return empty list
        case result {
          Ok(items) -> items
          Error(_) -> []
        }
...
// Jsonify the list of todo items
fn todos_to_json(items: List(Item)) {
  "["
  <> items
