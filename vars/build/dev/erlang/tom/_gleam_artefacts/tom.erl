...
-spec push_key({ok, BFK} | {error, get_error()}, binary()) -> {ok, BFK} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {BHP, list(binary())}} | {error, parse_error()})
) -> {ok, {BHP, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {BIA, list(binary())}} | {error, parse_error()},
    fun((BIA, list(binary())) -> {ok, BID} | {error, parse_error()})
) -> {ok, BID} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {BII, list(binary())}} | {error, parse_error()})
) -> {ok, {BII, list(binary())}} | {error, parse_error()}.
