...
-type message(KAD) :: {message, KAD} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(KAE, KAF) :: {continue,
        KAF,
        gleam@option:option(gleam@erlang@process:selector(KAE))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(KAG, KAH) :: {ready, KAG, gleam@erlang@process:selector(KAH)} |
    {failed, binary()}.

-type self(KAI, KAJ) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        KAI,
        gleam@erlang@process:subject(KAJ),
        gleam@erlang@process:selector(message(KAJ)),
        gleam@otp@system:debug_state(),
        fun((KAJ, KAI) -> next(KAJ, KAI))}.

-type spec(KAK, KAL) :: {spec,
        fun(() -> init_result(KAK, KAL)),
        integer(),
        fun((KAL, KAK) -> next(KAL, KAK))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(KAM) :: {ack,
        {ok, gleam@erlang@process:subject(KAM)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-spec continue(KAT) -> next(any(), KAT).
continue(State) ->
    {continue, State, none}.

-spec with_selector(next(KAX, KAY), gleam@erlang@process:selector(KAX)) -> next(KAX, KAY).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(KBJ))) -> gleam@erlang@process:selector(message(KBJ)).
...
-spec receive_message(self(any(), KBF)) -> message(KBF).
...
-spec init_selector(
    gleam@erlang@process:subject(KFX),
    gleam@erlang@process:selector(KFX)
) -> gleam@erlang@process:selector(message(KFX)).
...
-spec initialise_actor(
    spec(any(), KCA),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(KCA)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), KCN)) -> {ok, gleam@erlang@process:subject(KCN)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(KCT, fun((KCU, KCT) -> next(KCU, KCT))) -> {ok,
        gleam@erlang@process:subject(KCU)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(KDA), KDA) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-spec call(
    gleam@erlang@process:subject(KDC),
    fun((gleam@erlang@process:subject(KDE)) -> KDC),
    integer()
) -> KDE.
