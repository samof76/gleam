...
-spec method_override(gleam@http@request:request(HQH)) -> gleam@http@request:request(HQH).
...
-spec fn_with_bad_request_error(fun((HRI) -> {ok, HRJ} | {error, any()})) -> fun((HRI) -> {ok,
        HRJ} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((HRC, bitstring()) -> {ok, HRC} |
        {error, gleam@http@response:response(body())}),
    HRC
) -> {ok, {gleam@option:option(buffered_reader()), integer(), HRC}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), HSA})) -> list({binary(), HSA}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, HSD} | {error, any()},
    fun((HSD) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, HQU} | {error, any()}) -> {ok, HQU} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
