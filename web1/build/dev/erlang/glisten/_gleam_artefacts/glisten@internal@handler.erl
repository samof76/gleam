...
-type message(BEH) :: {internal, internal_message()} | {user, BEH}.

-type loop_message(BEI) :: {packet, bitstring()} | {custom, BEI}.

-type loop_state(BEJ, BEK) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(BEJ)),
        glisten@transport:transport(),
        BEK}.

-type connection(BEL) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(BEL))}.

-type handler(BEM, BEN) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(BEM), BEN, connection(BEM)) -> gleam@otp@actor:next(loop_message(BEM), BEN)),
        fun((connection(BEM)) -> {BEN,
            gleam@option:option(gleam@erlang@process:selector(BEM))}),
        gleam@option:option(fun((BEN) -> nil)),
        glisten@transport:transport()}.

-spec start(handler(BEX, any())) -> {ok,
        gleam@erlang@process:subject(message(BEX))} |
