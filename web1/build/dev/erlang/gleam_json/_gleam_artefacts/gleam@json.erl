...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JYB} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JYB} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JXX} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JXX} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, JXT} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, JXT} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(JYH), fun((JYH) -> json())) -> json().
...
-spec array(list(JYL), fun((JYL) -> json())) -> json().
