...
-spec set_header(request(JJJ), binary(), binary()) -> request(JJJ).
...
-spec set_query(request(JKF), list({binary(), binary()})) -> request(JKF).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(JKJ), gleam@http:method()) -> request(JKJ).
...
-spec set_scheme(request(JKQ), gleam@http:scheme()) -> request(JKQ).
set_scheme(Req, Scheme) ->
    erlang:setelement(5, Req, Scheme).

-spec set_host(request(JKT), binary()) -> request(JKT).
set_host(Req, Host) ->
    erlang:setelement(6, Req, Host).

-spec set_port(request(JKW), integer()) -> request(JKW).
set_port(Req, Port) ->
    erlang:setelement(7, Req, {some, Port}).

-spec set_path(request(JKZ), binary()) -> request(JKZ).
set_path(Req, Path) ->
    erlang:setelement(8, Req, Path).

-spec set_cookie(request(JLC), binary(), binary()) -> request(JLC).
...
-spec remove_cookie(request(JLH), binary()) -> request(JLH).
