...
-spec map_response_body(
    fun((JVP) -> gleam@http@response:response(JVB)),
    fun((JVB) -> JVC)
) -> fun((JVP) -> gleam@http@response:response(JVC)).
...
-spec prepend_response_header(
    fun((JVT) -> gleam@http@response:response(JVV)),
    binary(),
    binary()
) -> fun((JVT) -> gleam@http@response:response(JVV)).
...
-spec ensure_post(gleam@http@request:request(JVG)) -> {ok,
        gleam@http@request:request(JVG)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(JXC)) -> JXK)) -> fun((gleam@http@request:request(JXC)) -> JXK).
