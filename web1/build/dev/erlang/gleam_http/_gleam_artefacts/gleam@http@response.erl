...
-type response(JRC) :: {response, integer(), list({binary(), binary()}), JRC}.
...
-spec set_header(response(JRR), binary(), binary()) -> response(JRR).
...
-spec prepend_header(response(JRU), binary(), binary()) -> response(JRU).
prepend_header(Response, Key, Value) ->
    Headers = [{gleam@string:lowercase(Key), Value} |
        erlang:element(3, Response)],
    erlang:setelement(3, Response, Headers).

-spec set_body(response(any()), JRZ) -> response(JRZ).
set_body(Response, Body) ->
    {response, Status, Headers, _} = Response,
    {response, Status, Headers, Body}.

-spec try_map(response(JRD), fun((JRD) -> {ok, JRF} | {error, JRG})) -> {ok,
        response(JRF)} |
    {error, JRG}.
...
-spec map(response(JSB), fun((JSB) -> JSD)) -> response(JSD).
...
-spec expire_cookie(response(JSL), binary(), gleam@http@cookie:attributes()) -> response(JSL).
