...
-type valid_message(GEH) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, GEH}.

-type websocket_message(GEI) :: {valid, valid_message(GEI)} | invalid.
...
-type handler_message(GEJ) :: {internal, gramps@websocket:frame()} | {user, GEJ}.

-type websocket_state(GEK) :: {websocket_state,
        bitstring(),
        GEK,
...
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(GFS))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(GFS))).
...
-spec apply_frames(
    ...
    fun((GFI, websocket_connection(), handler_message(GFJ)) -> gleam@otp@actor:next(GFJ, GFI)),
    websocket_connection(),
    gleam@otp@actor:next(websocket_message(GFJ), GFI),
    fun((GFI) -> nil)
) -> gleam@otp@actor:next(websocket_message(GFJ), GFI).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {GET,
        gleam@option:option(gleam@erlang@process:selector(GEU))}),
    fun((GET) -> nil),
    fun((GET, websocket_connection(), handler_message(GEU)) -> gleam@otp@actor:next(GEU, GET)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok, gleam@erlang@process:subject(websocket_message(GEU))} | {error, nil}.
