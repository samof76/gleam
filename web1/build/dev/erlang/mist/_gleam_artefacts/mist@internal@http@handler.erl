...
-spec handle_file_body(
    ...
    mist@internal@http:connection(),
    mist@internal@http:http_version()
) -> {ok, gleam@http@response:response(gleam@bytes_builder:bytes_builder())} |
...
handle_file_body(Resp, Body, Conn, Http_version) ->
    ...
    Resp@1 = begin
        _pipe = Resp,
        _pipe@1 = gleam@http@response:set_body(_pipe, gleam@bytes_builder:new()),
...
handle_bytes_builder_body(Resp, Body, Conn, Req, Version) ->
    ...
    _pipe@2 = Resp@2,
    _pipe@3 = mist@internal@encoder:to_bytes_builder(
        _pipe@2,
        mist@internal@http:version_to_string(Version)
    ),
...
handle_chunked_body(Resp, Body, Conn, Version) ->
    ...
    gleam@result:replace(
        ...
        begin
            ...
            gleam@http@response:set_body(_pipe@10, gleam@bytes_builder:new())
        end
    ).
