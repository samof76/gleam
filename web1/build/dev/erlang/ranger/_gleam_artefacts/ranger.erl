...
-spec create(
    ...
    fun((FSM) -> FSM),
    fun((FSL, FSM) -> FSL),
    fun((FSL, FSL) -> gleam@order:order())
) -> fun((FSL, FSL, FSM) -> {ok, gleam@iterator:iterator(FSL)} | {error, nil}).
...
-spec create_infinite(
    fun((FSQ) -> boolean()),
    fun((FSQ, FSR) -> FSQ),
    fun((FSQ, FSQ) -> gleam@order:order())
) -> fun((FSQ, FSR) -> {ok, gleam@iterator:iterator(FSQ)} | {error, nil}).
