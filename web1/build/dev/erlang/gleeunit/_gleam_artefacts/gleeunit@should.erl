...
-spec equal(LVA, LVA) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(LVB, LVB) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, LVC} | {error, any()}) -> LVC.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, LVH}) -> LVH.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(LVK)) -> LVK.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
